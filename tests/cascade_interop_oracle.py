"""Compare the Rust author cascade with current Edge/Chromium."""
...
FIXTURE = ROOT / "tests" / "fixtures" / "interop" / "cascade_oracle.html"
...
def browser_contract(browser):
    with tempfile.TemporaryDirectory(prefix="render-cascade-oracle-") as profile:
        command = [
            browser,
            "--headless=new",
            "--disable-gpu",
    ...
    match = re.search(r'data-cascade-oracle="([^"]+)"', output)
    if not match:
        raise RuntimeError("browser dump did not contain cascade oracle data")
...
def rust_contract(property_names):
    ...
    result = cascade_html_snapshot(source, style_match.group(1), "#target")
...
def main():
    ...
    if actual != expected:
        print("Cascade interoperability contract differs")
        return 1
    print("Cascade interoperability contract matches")
    return 0
