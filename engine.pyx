...
# .pyx files are valid Python; this tells CPython's import machinery to
# treat them like .py sources so all rENDER modules can use .pyx extension.
# ---------------------------------------------------------------------------
...
# Add lib/ to path so `from gui import ...` works
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Try Qt first; fall back to pyx_gui (Win32) backend.
    try:
    ...
    # pyx_gui backend (Win32 native or headless no-op on other platforms)
    from backend.pyx_gui import register as _register_pyx
    _register_pyx()
    dl, page_h, _doc = _pipeline(
        *(_load_target(args.target)),
        viewport_width=args.width,
        viewport_height=args.height,
    )
    from backend.pyx_gui.painter import BrowserWindow
    win = BrowserWindow(dl, page_h, width=args.width,
                        height=args.height, title=_extract_title(_doc))
    win.run()
    return 0


def _load_target(target: str) -> tuple[str, str]:
    """Load HTML content from a file path or URL, return (html, base_url)."""
