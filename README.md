...
`rENDER` is a browser engine written in Python with `PyQt6`. It parses HTML, CSS, and a growing subset of JavaScript, computes styles, builds layout boxes, and paints real pages without delegating rendering to an existing browser engine.
...
```bash
python -m compileall engine.py css html js layout network rendering scripts tests
python -m ruff check engine.py css html js layout network rendering scripts tests
```
...
scripts/                 Fixture import and maintenance tooling
...
2. Hydrate known server-side compatibility payloads.
3. Fetch external CSS and image resources.
4. Execute supported JavaScript against the DOM.
5. Bind CSS, compute styles, and resolve layout boxes.
6. Paint the display list with the PyQt6 backend.
