...
def _parse_track_list(template: str, available: float) -> list[float]:
...
def layout_grid(node, container_box: BoxModel) -> BoxModel:
    """Layout a CSS grid container and its children."""
    from html.dom import Element
    from layout.block import layout_block, _parse_edge
    ...
    for child in children:
        ...
        if child_display == 'flex':
            ...
            cb = layout_flex(child, child_container)
        else:
