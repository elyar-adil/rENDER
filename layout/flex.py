...
def layout_flex(node, container_box: BoxModel) -> BoxModel:
    ...
    from layout.block import _parse_edge, layout_block
    ...
    box.x = container_box.x + margin.left + border_w.left + padding.left
    ...
    for child in flex_children:
        ...
        cb = layout_block(child, child_container)
        ...
        if remaining > 0 and total_grow > 0:
            ...
            for i, cb in enumerate(child_boxes):
                if flex_grows[i] > 0:
                    ...
                    # Re-layout child with new width
                    ...
                    child_container.content_height = 0.0
                    # Temporarily set a width style override
                    old_w = child.style.get('width', 'auto') if child.style else 'auto'
                    if child.style is None:
                        child.style = {}
                    child.style['width'] = f'{new_content_w}px'
                    new_cb = layout_block(child, child_container)
                    child.style['width'] = old_w
