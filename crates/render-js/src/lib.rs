...
/// Copied from `render-core` so this crate stays independent; it depends only
/// on `std` and `mp4parse`, and is consumed by `runtime::builtins::video`.
pub mod video;
...
mod tests {
    ...
    use render_dom::{MutationKind, NodeKind};
    use render_html::parse_document;

    fn element_with_id(dom: &render_dom::Dom, id: &str) -> render_dom::NodeId {
