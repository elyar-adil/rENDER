...
impl JsRuntime {
    pub(in crate::js::runtime) fn dispatch_promise_native(
        &mut self,
        dom: &mut Dom,
...
#[derive(Clone, Debug)]
pub(in crate::js::runtime) enum PromiseState {
    Pending,
    Fulfilled(JsValue),
    Rejected(JsValue),
}
...
impl JsRuntime {
    pub(in crate::js::runtime) fn create_promise(&mut self) -> Result<(usize, JsValue), JsError> {
    ...
    pub(in crate::js::runtime) fn resolve_promise(&mut self, promise: usize, value: &JsValue) {
        self.settle_promise(promise, value, true);
    }

    pub(in crate::js::runtime) fn resolve_promise_value(
        &mut self,
        promise: usize,
        value: &JsValue,
    ) -> Result<(), JsError> {
    ...
    pub(in crate::js::runtime) fn reject_promise(&mut self, promise: usize, reason: &JsValue) {
        self.settle_promise(promise, reason, false);
    }

    pub(in crate::js::runtime) fn settle_promise(
        &mut self,
        promise: usize,
        value: &JsValue,
        fulfilled: bool,
    ...
    pub(in crate::js::runtime) fn enqueue_reaction(
        &mut self,
    ...
    pub(in crate::js::runtime) fn perform_promise_then(
        &mut self,
        receiver: ObjectId,
        arguments: &[JsValue],
    ) -> Result<JsValue, JsError> {
