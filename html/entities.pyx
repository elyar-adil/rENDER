        ...
        if inner.startswith('#'):
            ...
            try:
                if num_str.lower().startswith('x'):
                    codepoint = int(num_str[1:], 16)
                else:
                    codepoint = int(num_str)
                # Guard against surrogates and invalid codepoints
            ...
            # Named entity (with or without trailing semicolon)
            return ENTITIES.get(inner, entity)

    # Match &name; or &name (without semicolon, for loose HTML)
    # and &#NNN; and &#xHHH;
    pattern = re.compile(r'&(#x?[0-9a-fA-F]+|#[0-9]+|[a-zA-Z][a-zA-Z0-9]*);?')
